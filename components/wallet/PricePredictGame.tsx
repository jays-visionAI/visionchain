                    ...
                    <div class="flex flex-col items-center gap-3">
                        <Show when={props.playsRemaining && props.playsRemaining() <= 0 && props.onInviteFriend}>
                            <button onClick={() => props.onInviteFriend!()}
                                class="w-full px-6 py-3 bg-gradient-to-r from-emerald-500/20 to-green-500/20 border border-emerald-500/30 rounded-xl text-sm font-bold text-emerald-400 hover:bg-emerald-500/25 active:scale-95 transition-all flex items-center justify-center gap-2">
                        ...
                        <div class="flex gap-3 justify-center">
                            <button onClick={() => { setGameOver(false); setTotalPlays(0); setTotalCorrect(0); setStreak(0); startRound(); }}
                                ...
                                class={`px-6 py-3 border rounded-xl text-sm font-bold transition-all ${props.playsRemaining && props.playsRemaining() <= 0 ? 'bg-gray-800/50 border-gray-700/30 text-gray-600 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-500/20 to-violet-500/20 border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/30'}`}>
