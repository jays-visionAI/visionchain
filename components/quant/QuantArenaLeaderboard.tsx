...
import type { Competition, CompetitionEntry } from '../../services/quant/types';
import {
...
export const QuantArenaLeaderboard = (): JSX.Element => {
    const [competition, setCompetition] = createSignal<Competition | null>(null);
    const [entries, setEntries] = createSignal<CompetitionEntry[]>([]);
    ...
    return (
        <div class="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <Show when={!loading() && competition()}>
                {/* ─── Round Header Banner ─── */}
                <div class="relative overflow-hidden rounded-3xl border border-cyan-500/15 bg-gradient-to-br from-[#0a1628] via-[#111113] to-[#0f1a2e]">
                    {/* Background glow */}
                    <div class="absolute top-0 right-0 w-64 h-64 bg-cyan-500/[0.06] rounded-full blur-[80px] -mr-32 -mt-32" />
                    <div class="absolute bottom-0 left-0 w-48 h-48 bg-purple-500/[0.04] rounded-full blur-[60px] -ml-24 -mb-24" />
                    ...
                    <div class="relative p-6">
                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <div class="flex items-center gap-2 mb-1.5">
                                    <div class="px-2 py-0.5 bg-cyan-500/15 border border-cyan-500/25 rounded text-[9px] font-black text-cyan-400 uppercase tracking-wider">
                                        Live
                                    </div>
                                    <span class="text-[10px] text-gray-500">Round 1</span>
                                </div>
                                <h2 class="text-xl font-black text-white tracking-tight">{competition()!.title}</h2>
                                <p class="text-[11px] text-gray-400 mt-1 max-w-md">{competition()!.description}</p>
                            </div>
                            <button
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex items-center justify-between mt-1">
                                <span class="text-[9px] text-gray-600">
                                    {new Date(competition()!.startDate).toLocaleDateString('en', { month: 'short', day: 'numeric' })}
                                </span>
                                <span class="text-[9px] text-gray-600">
                                    {new Date(competition()!.endDate).toLocaleDateString('en', { month: 'short', day: 'numeric' })}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <div class="p-3 bg-white/[0.03] rounded-xl border border-white/[0.04]">
                                ...
                                <div class="text-base font-black text-white">{competition()!.participantCount}</div>
                            </div>
                            <div class="p-3 bg-white/[0.03] rounded-xl border border-white/[0.04]">
                            ...
                            <div class="p-3 bg-white/[0.03] rounded-xl border border-white/[0.04]">
                                ...
                                <div class="text-base font-black text-cyan-400">9K VCN</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <Show when={showRules()}>
                    <div class="p-5 bg-[#111113] rounded-2xl border border-white/[0.06] space-y-3">
                        ...
                        <div class="mt-3 pt-3 border-t border-white/[0.04]">
                            <h5 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Prize Structure</h5>
                            <div class="space-y-1.5">
                                <For each={competition()!.prizes}>
                                    {(prize) => (
                                        <div class="flex items-center justify-between px-3 py-2 bg-white/[0.02] rounded-lg">
                                            ...
                                            <span class={`text-[11px] font-black ${prize.rankMin <= 3 ? 'text-cyan-400' : 'text-gray-500'}`}>{prize.reward}</span>
                                        </div>
                                    )}
                                </For>
                            </div>
                ...
                {/* ─── My Status Card ─── */}
                <Show when={myEntry()}>
                    <div class="p-4 bg-gradient-to-r from-cyan-500/[0.06] to-purple-500/[0.04] rounded-2xl border border-cyan-500/15">
                        <div class="flex items-center gap-2 mb-3">
                            <ChartUpIcon />
                            <span class="text-[11px] font-bold text-cyan-400 uppercase tracking-wider">My Position</span>
                        </div>
                ...
                <div class="bg-[#111113] rounded-2xl border border-white/[0.06] overflow-hidden">
                    ...
                    <div class="flex items-center justify-between px-5 py-3 border-b border-white/[0.04]">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="text-xs font-black text-white uppercase tracking-wider">Leaderboard</h3>
                            <span class="text-[9px] text-gray-500 bg-white/[0.04] px-1.5 py-0.5 rounded">{entries().length} traders</span>
                        </div>
                    </div>
                    ...
                    <Show when={entries().length === 0}>
                        <div class="flex flex-col items-center justify-center py-12">
                            ...
                            <p class="text-xs text-gray-500 mt-3">No participants yet. Be the first to compete!</p>
                        </div>
                    </Show>
                        ...
                        <For each={entries()}>
                            {(entry) => {
                                ...
                                const rankColor = () => {
                                    ...
                                    if (isMe()) return 'bg-cyan-500/[0.06] border-l-2 border-l-cyan-400';
                                ...
                                return (
                                    <div class={`grid grid-cols-12 gap-2 items-center px-5 py-3 border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors ${rankColor()}`}>
                                        ...
                                        <div class="col-span-3">
                                            <div class={`text-xs font-bold ${isMe() ? 'text-cyan-400' : 'text-white'} truncate`}>
                ...
                {/* ─── How to Participate ─── */}
                <Show when={!myEntry()}>
                    <div class="p-5 bg-gradient-to-r from-cyan-500/[0.04] to-purple-500/[0.03] rounded-2xl border border-cyan-500/10">
                        <div class="flex items-center gap-2 mb-3">
                            <TrophyIcon class="w-4 h-4 text-cyan-400" />
                            <span class="text-xs font-bold text-white">Join the Arena</span>
                        </div>
                        <p class="text-[11px] text-gray-400 leading-relaxed mb-3">
                            Go to the Strategies tab, select a strategy, configure your agent in Paper Trading mode, and you'll be automatically enrolled in the competition.
                        </p>
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-1.5">
                                <div class="w-5 h-5 rounded-full bg-cyan-500/15 flex items-center justify-center">
                                    <span class="text-[9px] font-black text-cyan-400">1</span>
                                </div>
                                <span class="text-[10px] text-gray-500">Select Strategy</span>
                            </div>
