    ...
    const fetchAllNodes = async () => {
        ...
        try {
            ...
            mobileSnap.forEach((doc) => {
                ...
                let hbTime = 0;
                if (lastHb) {
                    hbTime = lastHb.toDate ? lastHb.toDate().getTime() : new Date(lastHb).getTime();
                }
                const online = hbTime > tenMinAgo();
                if (online) onlineCount++;

                const status = d.status || 'inactive';
                if (status === 'active') { activeCount++; byStatus.active++; }
                else if (status === 'suspended') byStatus.suspended++;
                else byStatus.inactive++;

                byType.mobile++;
                const dt = (d.device_type || '').toLowerCase();
                if (dt === 'android') byPlatform.android++;
                else if (dt === 'ios') byPlatform.ios++;
                else if (dt === 'pwa') byPlatform.pwa++;
                ...
                allNodes.push({
                    id: doc.id, email: d.email || '', type: 'mobile',
                    nodeClass, platform: d.device_type || 'unknown',
                    status, currentMode: d.current_mode || 'offline',
                    ...
                    storageUsedMB: 0, chunkCount: 0, proofsPassed: 0, proofsFailed: 0,
                    lastHeartbeat: lastHb, createdAt: d.created_at, isOnline: online,
                });
            });

            // 2. Fetch vision_nodes (desktop nodes)
            try {
                ...
                visionSnap.forEach((doc) => {
                    ...
                    let hbTime = 0;
                    if (lastHb) {
                        hbTime = typeof lastHb === 'number' ? lastHb : (lastHb.toDate ? lastHb.toDate().getTime() : new Date(lastHb).getTime());
                    }
                    const online = hbTime > tenMinAgo();
                    if (online) onlineCount++;
                    ...
                    allNodes.push({
                        ...
                        nodeClass, platform: d.platform || 'unknown',
                        status, currentMode: d.mode || 'full',
                        uptimeSeconds: d.total_uptime_seconds || 0,
                        heartbeatCount: d.heartbeat_count || 0, streakDays: d.streak_days || 0,
                        pendingReward: pending, totalEarned: earned, claimedReward: claimed,
                        storageUsedMB: storageMB, chunkCount: chunks,
                        proofsPassed: pp, proofsFailed: pf,
                        lastHeartbeat: lastHb, createdAt: d.created_at, isOnline: online,
                    });
                });
            } catch (e) {
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <StatCard icon="active" label="Active" value={stats().activeNodes}
                    sub={`${stats().totalNodes > 0 ? Math.round((stats().activeNodes / stats().totalNodes) * 100) : 0}% of total`} color="bg-green-500/10" />
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex-1 min-w-[200px]">
                    <input
                        type="text"
                        placeholder="Search by ID, email, or platform..."
                        value={searchQuery()}
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-white/5">
                                ...
                                <th class="px-3 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-gray-500">Type</th>
                                ...
                                <For each={filteredNodes()}>
                                    {(node) => (
                                        <tr class="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                                            <td class="px-3 py-3">
                                                <div class="flex items-center gap-2">
                                                    <div class={`w-2 h-2 rounded-full ${node.isOnline ? 'bg-green-400 shadow-[0_0_6px_rgba(74,222,128,0.5)]' : 'bg-gray-600'}`} />
                                                    <div>
                                                        <div class="text-xs font-semibold text-gray-200 truncate max-w-[120px]">{node.id.slice(0, 12)}...</div>
                                                        <div class="text-[10px] text-gray-600 truncate max-w-[120px]">{node.email || '-'}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-3 py-3">
                                                <span class={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase rounded-md ${node.type === 'mobile' ? 'bg-blue-500/10 text-blue-400' : 'bg-purple-500/10 text-purple-400'}`}>
                                                    {node.type}
                                                </span>
                                                <div class="text-[10px] text-gray-600 mt-0.5">{node.nodeClass}</div>
                                            </td>
                                            <td class="px-3 py-3">
                                            ...
                                            <td class="px-3 py-3 text-xs text-cyan-400 font-mono">{node.storageUsedMB > 0 ? formatStorage(node.storageUsedMB) : '-'}</td>
                                            <td class="px-3 py-3 text-xs text-gray-500">{timeAgo(node.lastHeartbeat)}</td>
                                        </tr>
                                    )}
                                </For>
                                <Show when={filteredNodes().length === 0}>
                                    <tr><td colspan="9" class="px-4 py-12 text-center text-gray-600">No nodes found</td></tr>
                                </Show>
                            </Show>
                        </tbody>
                    </table>
