...
const Wallet = (): JSX.Element => {
    ...
    const executePendingAction = async () => {
        ...
        try {
            ...
            if (action.type === 'purchase_node') {
                ...
                if (isSchedulingTimeLock()) {
                    ...
                    try {
                        ...
                        const [tokenName, nonce] = await Promise.all([
                            sepoliaVcnContract.name(),
                            sepoliaVcnContract.nonces(walletAddress())
                        ]);

                        const permitSignature = await signer.signTypedData(
                            { name: tokenName, version: '1', chainId: 11155111, verifyingContract: SEPOLIA_VCN_TOKEN },
                            {
                                Permit: [
                ...
                // 2. Detect bridge direction and execute accordingly
                const bridgeData = action.data as any;
                if (bridgeData.bridgeDirection === 'reverse' || bridgeData.bridgeType === 'REVERSE_BRIDGE') {
                    // Reverse bridge: Sepolia VCN → Vision Chain VCN
                    await executeReverseBridgeIntent({
                    ...
                    await executeBridgeIntent({
                        amount: bridgeData.amount || ethers.formatEther(BigInt(bridgeData.amount || '0')),
                        destinationChain: bridgeData.destinationChain || 'SEPOLIA',
                        recipient: bridgeData.recipient,
                    });
                }
            }
    ...
    const fetchPortfolioData = async () => {
        ...
        try {
            ...
            if (walletAddress()) {
                ...
                // Also fetch Sepolia wVCN balance and sync to userHoldings
                await fetchSepoliaBalance();
                setUserHoldings(prev => ({
                    ...prev,
                    VCN_SEPOLIA: sepoliaVcnBalance()
                }));
            }
        } catch (error) {
    ...
    const getAssetData = (symbol: string): AssetData => {
        ...
        const staticPrices: Record<string, { name: string, price: number, image?: string }> = {
            ...
            'VCN_SEPOLIA': { name: 'Sepolia VCN', price: getVcnPrice() }
        };
    ...
    const executeReverseBridgeIntent = async (bridge: { amount: string; sourceChain: string; recipient?: string; privateKey?: string }) => {
        ...
        try {
            ...
            const bridgeAmount = parseFloat(bridge.amount);
            const bridgeFee = 1; // 1 VCN Sepolia fee
            if (currentSepoliaBalance < bridgeAmount + bridgeFee) {
                throw new Error(lastLocale() === 'ko'
                    ? `Sepolia VCN 잔고 부족. 보유: ${currentSepoliaBalance.toFixed(2)}, 필요: ${(bridgeAmount + bridgeFee).toFixed(2)} (전송액 + 수수료)`
                    : `Insufficient Sepolia VCN balance. Have: ${currentSepoliaBalance.toFixed(2)}, Need: ${(bridgeAmount + bridgeFee).toFixed(2)} (amount + fee)`);
            }
            ...
            setBridgeTasks(prev => [
                {
                    id: optimisticId,
                    type: 'BRIDGE' as const,
                    summary: `${bridge.amount} VCN Sepolia → Vision Chain`,
                    status: 'WAITING' as const,
                    timestamp: Date.now(),
                    recipient: bridge.recipient || 'Bridge',
                    amount: bridge.amount,
            ...
            // Resolve recipient
            let resolvedRecipientAddr = userAddr;
            if (bridge.recipient) {
                if (ethers.isAddress(bridge.recipient)) {
                    resolvedRecipientAddr = bridge.recipient;
                } else {
                    ...
                    if (resolved && resolved.address) {
                        ...
                        throw new Error(lastLocale() === 'ko'
                            ? `"${bridge.recipient}" 수신자를 찾을 수 없습니다.`
                            : `Recipient "${bridge.recipient}" not found.`);
                    }
                }
            }
            ...
            console.error('[ReverseBridge] Error:', error);
            setBridgeTasks(prev => prev.filter(t => !t.id.startsWith('rbridge_optimistic_')));
            const errMsg = lastLocale() === 'ko'
                ? `역방향 브릿지 실패: ${error.message || '알 수 없는 오류'}`
                : `Reverse bridge failed: ${error.message || 'Unknown error'}`;
            setMessages(prev => [...prev, {
                role: 'assistant',
                content: errMsg
            }]);
