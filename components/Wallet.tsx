    ...
    const executePendingAction = async () => {
        ...
        try {
            ...
            if (action.type === 'purchase_node') {
                ...
                setLoadingMessage('AGENT: PROCESSING BATCH...');
                setFlowLoading(true);
                ...
                for (let i = 0; i < transactions.length; i++) {
                    ...
                    // Clean amount and specific validations
                    const rawAmount = tx.amount ? String(tx.amount).replace(/[^0-9.]/g, '') : "0";
                    const recipient = tx.address || tx.recipient;

                    setLoadingMessage(`AGENT: EXECUTING ${i + 1}/${transactions.length}...`);
                    ...
                    try {
                        ...
                        // Try VCN transfer
                        const receipt = await contractService.sendTokens(recipient, rawAmount, 'VCN');
                        hash = receipt.hash;

                        results.push({ ...tx, status: 'success', hash });
                        successCount++;
                    } catch (e: any) {
                        console.error(`Batch Tx ${i + 1} Failed:`, e);
                        results.push({ ...tx, status: 'failed', error: e.message || String(e) });
                        failCount++;
                    }
                }
                ...
                setBatchAgents(prev => [...prev, {
                    id: batchId,
                    type: 'BATCH', // AgentType
                    status: 'SENT',
                    summary: `${successCount}/${transactions.length} Processed`,
                ...
                setReviewMulti(null); // Close review panel
                setShowPasswordModal(false);
                setFlowLoading(false);

                if (failCount > 0) {
                    alert(`Batch Execution Completed.\n\nSuccess: ${successCount}\nFailed: ${failCount}\n\nCheck console for details.`);
                } else {
                    setLoadingMessage('BATCH AGENT: DONE');
