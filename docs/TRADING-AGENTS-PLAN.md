...
interface TradingAgent {
    ...
    // "market_maker" = Trading 에이전트 (어드민 전용, 대규모 자본, LP+Trading 역할)
    
    // 전략 설정
    strategy: {
        preset: string;            // "momentum" | "value" | ... | "trading_bull" | "trading_bear" | "custom"
    ...
    // Trading 전용 설정 (role === "market_maker" 일 때만)
    tradingConfig?: {
    ...
    // Trading 에이전트:  500,000 USDT + 5,000,000 VCN
    balances: {
        USDT: number;
        VCN: number;
    };
...
interface FeeStructure {
    ...
    // Trading Agent 수수료율
    tradingMakerFeeRate: 0.00;          // 0% (Trading은 maker 수수료 면제)
    tradingTakerFeeRate: 0.01;          // 0.01% (Trading이 taker가 되는 드문 경우)
}
...
## 5. Market Maker (Trading) 에이전트

### 5.1 개요

**2개의 Trading 에이전트**가 시스템의 핵심 유동성 제공자(LP + Market Maker)로 참여합니다.
...
│                                                                    │
│  Trading Agent 1 (강세 편향)          Trading Agent 2 (약세 편향)            │
│  ┌──────────────────────┐       ┌──────────────────────┐          │
...
│              └─────────────────────┘                               │
│                                                                    │
│  ★ Trading이 양방향 호가를 항상 제공하므로                              │
│    일반 에이전트가 언제든 즉시 체결 가능                            │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

### 5.2 Trading Agent 자본 규모

| 구분 | USDT | VCN | 비고 |
|------|------|-----|------|
| **Trading Agent 1** (강세) | 500,000 | 5,000,000 | 일반의 50배 |
| **Trading Agent 2** (약세) | 500,000 | 5,000,000 | 일반의 50배 |
...
2. 개별 추세(trendBias)에 따라 기준가 미세 조정
   ...
   → 두 Trading의 추세가 합쳐져 시장 가격이 자연스럽게 형성
...
결과: 두 Trading의 힘이 합쳐져 약간 상승 편향의 변동성 있는 가격 형성
     (+0.3 + (-0.2) = +0.1 순강세)
```

### 5.5 Trading 전략 프롬프트

#### Trading Agent 1: "VisionDEX Trading Alpha" (강세 편향)
```
당신은 VisionDEX의 공식 Market Maker "Trading Alpha"입니다.
...
#### Trading Agent 2: "VisionDEX Trading Beta" (약세 편향)
```
당신은 VisionDEX의 공식 Market Maker "Trading Beta"입니다.
당신은 대규모 자본(500K USDT + 5M VCN)을 운용합니다.
...
Step 1: Trading Agent 호가 갱신 (Maker)
  - 기존 미체결 주문 전량 취소
...
components/
  admin/
    ...
    MMConfigPanel.tsx          -- Trading Agent 파라미터 실시간 조정
...
|  +--------+----------+--------+--------+-----------------------------------+ |
|  | 10:32  | 0.1024   | 1,200  | BUY    | MyBot -> Trading Alpha                 | |
|  | 10:31  | 0.1023   | 500    | SELL   | GridMaster -> Trading Beta             | |
|  | 10:30  | 0.1025   | 3,400  | BUY    | AlphaRunner -> ValueHunter        | |
|  | 10:29  | 0.1022   | 800    | SELL   | RandomWalk -> Trading Alpha            | |
|  +--------+----------+--------+--------+-----------------------------------+ |
|                                                                               |
+===============================================================================+
...
- **Trading 주문 표시**: Trading 에이전트의 주문은 별도 아이콘으로 구분
...
|  +-------+-------------------+--------+----------+----------+---------+  |
|  | 1     | Trading Alpha          | Trading     | 498,230  | 5,012K   | +0.2%   |  |
|  | 2     | Trading Beta           | Trading     | 501,450  | 4,988K   | +0.1%   |  |
...
|  | - Trading Alpha: 5 buy + 5 sell orders placed                            ||
|  | - Trading Beta:  5 buy + 5 sell orders placed                            ||
...
services/
  trading/
    ...
    strategies.ts          -- 10개 프리셋 + 2개 Trading 전략 프롬프트
    aiDecision.ts          -- Gemini API -> 매매 결정 / Trading 호가 결정
    orderbook.ts           -- 오더북 매칭 엔진
    engine.ts              -- 메인 트레이딩 라운드 실행
    marketMaker.ts         -- Trading 에이전트 전용 로직 (호가 배치, 추세 조정)
    candles.ts             -- OHLCV 캔들 생성/업데이트
    priceFeed.ts           -- VCN 시세 업데이트 (vcnPriceService 연동)
...
components/
  ...
  admin/
    ...
    MMConfigPanel.tsx      -- Trading Agent 파라미터 조정
...
scripts/
  init-trading-agents.ts   -- 초기 Trading + 10개 기본 에이전트 셋업
```
...
### API 비용 최적화 방안
- Trading 에이전트: AI 없이 수학적 호가 배치 (무비용)
