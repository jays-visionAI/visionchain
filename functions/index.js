  ...
  async (event) => {
    ...
    try {
      const stuckSnapshot = await db.collection("scheduledTransfers")
        ...
        .where("lockExpiresAt", "<", nowTs) // Requires index? If so, query status only and filter in memory
        .limit(50)
        .get();
      ...
      const stuckDocs = stuckSnapshot.docs.filter(d => {
        const dData = d.data();
        return dData.lockExpiresAt && dData.lockExpiresAt.toMillis() < nowMillis;
      });

      if (stuckDocs.length > 0) {
        console.log(`Recovering ${stuckDocs.length} stuck jobs (Manual Filter)...`);
        const batch = db.batch();
