...
/**
 * @title VCNVestingNative
 * @dev Vesting contract for native VCN token distribution.
 * Modified for Vision Chain v2 where VCN is the native token (gas currency).
 * 
 * Features:
 * - Cliff period (e.g., 3 months)
 * - Monthly unlock (e.g., 5% per month)
 * - Uses native VCN instead of ERC-20
 */
contract VCNVestingNative is AccessControl, ReentrancyGuard {
    ...
    /**
     * @dev Fund the contract with native VCN for vesting payouts
     */
    ...
    /**
     * @dev Claim vested native VCN tokens
     */
    ...
    /**
     * @dev Get contract's native VCN balance
     */
