...
async function main() {
    ...
    const server = http.createServer(async (req, res) => {
        if (req.url === "/health" || req.url === "/") {
        ...
        } else if (req.url === "/test-upbit") {
            // Diagnostic: Test Upbit API accessibility from GCP
            const axios = require("axios");
            ...
            try {
                const resp = await axios.get("https://api.upbit.com/v1/ticker?markets=KRW-BTC", { timeout: 10000 });
                results.tests.publicTicker = { status: "OK", price: resp.data?.[0]?.trade_price };
            } catch (err) {
                results.tests.publicTicker = { status: "BLOCKED", code: err.response?.status, message: err.message };
            }

            // Test 2: Public API with User-Agent
            try {
                const resp = await axios.get("https://api.upbit.com/v1/ticker?markets=KRW-BTC", {
                    timeout: 10000,
                    headers: { "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)" }
                });
                results.tests.publicTickerWithUA = { status: "OK", price: resp.data?.[0]?.trade_price };
            } catch (err) {
                results.tests.publicTickerWithUA = { status: "BLOCKED", code: err.response?.status, message: err.message };
            }

            // Test 3: Authenticated API (/v1/accounts) - non-destructive
            const upbitAgent = agents.find(a => (a.exchange || "upbit") === "upbit");
            if (upbitAgent) {
                const keyId = upbitAgent.keyId || upbitAgent.exchangeKeyId;
                if (keyId) {
                    try {
                        const keySnap = await db.collection("exchangeKeys").doc(keyId).get();
                        if (keySnap.exists && keySnap.data()?.encryptedKey) {
                            const keyData = keySnap.data();
                            const accessKey = decryptApiKey(keyData.encryptedKey);
                            const secretKey = decryptApiKey(keyData.encryptedSecret);
                            const payload = { access_key: accessKey, nonce: uuidv4() };
                            const token = jwt.sign(payload, secretKey);

                            const resp = await axios.get("https://api.upbit.com/v1/accounts", {
                                headers: { Authorization: `Bearer ${token}` },
                                timeout: 10000,
                            });
                            results.tests.authenticatedAccounts = {
                                status: "OK",
                                accountCount: resp.data?.length || 0,
                                currencies: (resp.data || []).map(a => a.currency).slice(0, 5),
                            };
                        } else {
                            results.tests.authenticatedAccounts = { status: "SKIPPED", reason: `Key doc ${keyId} not found or empty` };
                        }
                    } catch (err) {
                        results.tests.authenticatedAccounts = {
                            status: "FAILED",
                    ...
                    // Fallback: Use test env vars if available
                    const testAK = process.env.TEST_UPBIT_AK;
                    const testSK = process.env.TEST_UPBIT_SK;
                    if (testAK && testSK) {
                        try {
                            const payload = { access_key: testAK, nonce: uuidv4() };
                            const token = jwt.sign(payload, testSK);
                            const resp = await axios.get("https://api.upbit.com/v1/accounts", {
                                headers: { Authorization: `Bearer ${token}` },
                                timeout: 10000,
                            });
                            results.tests.authenticatedAccounts = {
                                ...
                                source: "env-vars",
                                accountCount: resp.data?.length || 0,
                                currencies: (resp.data || []).map(a => `${a.currency}: ${a.balance}`).slice(0, 10),
                            };
                        } catch (err) {
                            results.tests.authenticatedAccounts = {
                                status: "FAILED",
                        ...
                        results.tests.authenticatedAccounts = {
                            status: "SKIPPED",
                            reason: "No keyId on agent and no TEST_UPBIT_AK/SK env vars",
                        };
                    }
                }
            } else {
                results.tests.authenticatedAccounts = { status: "SKIPPED", reason: "No Upbit agent found" };
            }

            // Test 4: Check outgoing IP
            try {
                const ipResp = await axios.get("https://api.ipify.org?format=json", { timeout: 5000 });
                results.outgoingIp = ipResp.data?.ip;
            } catch { results.outgoingIp = "unknown"; }
