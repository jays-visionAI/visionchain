/**
 * Bridge Finalizer Runner
 * 
 * Monitors Vision Chain MessageInbox for pending messages
 * Finalizes after challenge period and triggers mint
 */
...
const config = {
    ...
    dstEqualizer: process.env.DST_EQUALIZER,
...
class BridgeFinalizer {
    ...
    async checkAndFinalize() {
        ...
        console.log(`\n[${new Date().toISOString()}] Checking ${count} pending messages...`);
        ...
        for (let i = 0n; i < count; i++) {
            ...
            if (timeRemaining === 0n) {
                console.log(`\nFinalizing: ${messageHash.slice(0, 20)}...`);
...
const finalizer = new BridgeFinalizer();
